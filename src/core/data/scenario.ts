...
export function buildScenario(ctx: Ctx): ScenarioData {
  ...
  const millerBase = order('ord_miller_frame', miller.id, 'Patio base & bedding', 'plan', {
    inDays: 9,
    sortOrder: 0,
  });
  line(millerBase.id, 'AGG-HPB-BULK', 34, 'Triaxle to the back lane — tight access');
  line(millerBase.id, 'AGG-SCREEN-BULK', 9);
  line(millerBase.id, 'GEO-FABRIC-RL', 1);
  line(millerBase.id, 'EDG-PVC-STD', 96);

  // The surface, and the special-order line that forces the quote desk.
  const millerSurface = order('ord_miller_deck', miller.id, 'Paver field & coping', 'plan', {
    inDays: 16,
    sortOrder: 1,
  });
  ...
  const salesOrders: SalesOrder[] = [
    ...
    seedSalesOrder(SO_KIRKLAND, kirklandTerrace, 'SO-5044', 'invoiced', 40, [
  ...
  const kirklandTotal = items
    .filter((item) => item.orderId === kirklandTerrace.id)
    .reduce((sum, item) => sum + Math.round((item.unitPrice ?? 0) * item.qty), 0);
